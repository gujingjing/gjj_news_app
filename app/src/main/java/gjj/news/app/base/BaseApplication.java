...
import gjj.android.base.netstate.NetChangeObserver;
import gjj.android.base.netstate.NetworkStateReceiver;
...
//        initSPUtils();
    ...
    public void initLogUtils(Boolean log,String tag) {
        // 配置日志是否输出(默认true)
        LogUtils.setIfLog(log);

        // 配置日志前缀
        LogUtils.customTagPrefix = tag;
    }
    public void initSPUtils(String sp_name){
        //初始化配置sp文件,注意，只是配置了，但是有最新的diskCash进行保存
        SharedPrefsUtil.SHARE_SP_NAME="sp_name";
    }
    ...
    public void onDisConnect() {
        ...
        if (mCurrentActivity != null) {
            ...
            }else if(mCurrentActivity instanceof MvpBaseActivity){
                ((MvpBaseActivity) mCurrentActivity).onDisConnect();
            }
        }
    }
    ...
    protected void onConnect(NetWorkUtil.NetType type) {
        ...
        if (mCurrentActivity != null) {
            if (mCurrentActivity instanceof BaseActivity) {
                ...
                ((MvpBaseActivity) mCurrentActivity).onConnect(type);
            }
        }
    }
}
